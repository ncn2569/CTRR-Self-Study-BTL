...
Cho một mặt bằng tòa nhà dạng lưới $m \times n$ (0 = ô trống, 1 = tường). Tìm đường sơ tán ngắn nhất từ vị trí \texttt{(startX, startY)} đến lối thoát \texttt{(exitX, exitY)}. Bài toán giống Task 3 nhưng có thêm yêu cầu output bổ sung.

\textbf{Input:} Ma trận lưới \texttt{floorPlan}, kích thước $m \times n$, tọa độ start, tọa độ exit, ma trận \texttt{weightMatrix} (truyền vào để ghi kết quả), mode heuristic.

\textbf{Output:}
\begin{itemize}
    \item Linked list \texttt{PathNode} chứa các bước di chuyển (giống Task 3)
    \item \texttt{weightMatrix[x][y]}: sau khi tìm được đường đi, mỗi ô nằm trên path được ghi giá trị $g(x,y)$ vào ma trận; các ô không nằm trên path giữ nguyên bằng 0
\end{itemize}

\textbf{Hàm chi phí:} Giống Task 3 (thẳng = 1.0, chéo = 1.5; Mode 1 Manhattan, Mode 2 Chebyshev).

\subsection{Cách điền weightMatrix}

Trong vòng truy vết ngược từ Goal về Start:

\begin{lstlisting}[caption={Ghi weightMatrix trong findEvacuationPath -- Algo.cpp}]
int i = goalPoint;
while (parents[i] != -1) {
    path.push_back(parents[i]);
    int x = parents[i] / 100;
    int y = parents[i] % 100;
    weightMatrix[x][y] = g[parents[i]]; // ghi g cho o trong path
    i = parents[i];
}
// ghi them o goal
int x = goalPoint / 100, y = goalPoint % 100;
weightMatrix[x][y] = g[goalPoint];
\end{lstlisting}

\subsection{Chạy tay ví dụ}

\textbf{FloorPlan 4x4, tường tại (1,1), Start=(0,0), Goal=(2,2), Mode 1:}

\begin{lstlisting}[language={}, numbers=none]
     Col: 0  1  2  3
Row 0: [  0  0  0  0  ]  <- Start (0,0)
Row 1: [  0  W  0  0  ]  <- tuong tai (1,1)
Row 2: [  0  0  0  0  ]  <- Goal (2,2)
Row 3: [  0  0  0  0  ]
\end{lstlisting}

$h(\text{start}) = |0-2|+|0-2| = 4$, $f_0 = 4$.

Các đường khả thi:
\begin{itemize}
    \item $(0,0) \to (1,0) \to (2,1) \to (2,2)$: $g = 1 + 1.5 + 1 = 3.5$ (tối ưu)
    \item $(0,0) \to (0,1) \to (0,2) \to (1,2) \to (2,2)$: $g = 1+1+1+1 = 4.0$
\end{itemize}

A* tìm ra đường $g=3.5$.
...
\textbf{WeightMatrix sau khi chạy:}

\begin{table}[H]
\centering
\begin{tabular}{ccl}
\toprule
\textbf{Ô} & \textbf{weightMatrix} & \textbf{Lý do} \\
\midrule
$(0,0)$ & 0   & Start, reset\_double để 0 từ đầu \\
$(1,0)$ & 1.0 & Nằm trong path, ghi bởi vòng while \\
$(2,1)$ & 2.5 & Nằm trong path, ghi bởi vòng while \\
$(2,2)$ & 3.5 & Goal, ghi sau vòng while \\
Còn lại & 0   & Không nằm trong path \\
\bottomrule
\end{tabular}
\caption{Kết quả weightMatrix sau Task 4}
\end{table}

\subsection{Code hiện thực ghi weightMatrix}
\begin{lstlisting}[caption={Hàm findEvacuationPath xử lý weightMatrix}]
